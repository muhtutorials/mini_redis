...
impl Subscribe {
    ...
    pub(crate) async fn apply(
        ...
        loop {
            // "self.channels" is used to track additional channels to subscribe to.
            // When new "SUBSCRIBE" commands are received during the
            // execution of "apply", the new channels are pushed into this vector.
            for channel_name in self.channels.drain(..) {
                subscribe_to_channel(channel_name, &mut subscriptions, db, conn).await?;
            }
            ...
            select! {
                ...
                Some((channel_name, msg)) = subscriptions.next() => {
                    conn.write_frame(&make_message_frame(channel_name, msg)).await?;
                }
                resp = conn.read_frame() => {
                    ...
                    handle_command(frame, &mut self.channels, &mut subscriptions, conn).await?;
                }
                _ = shutdown.recv() => {
                    return Ok(());
                }
...
//
// All of these functions take the channel name as a "String" instead of
...
fn make_subscribe_frame(channel_name: String, num_subs: usize) -> Frame {
    ...
    frame.push_bulk(Bytes::from(channel_name));
    ...
    pub(crate) fn parse_frames(parse: &mut Parse) -> Result<Unsubscribe, ParseError> {
        ...
        loop {
            match parse.next_string() {
                ...
                Ok(s) => channels.push(s),
                // the "EndOfStream" error indicates that there is no further data to parse
                Err(ParseError::EndOfStream) => break,
                // all other errors are bubbled up, resulting in the connection
                // being terminated
                Err(err) => return Err(err.into()),
