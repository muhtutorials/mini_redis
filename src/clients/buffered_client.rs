...
async fn run(mut client: Client, mut rx: Receiver<Message>) {
    ...
    while let Some((cmd, tx)) = rx.recv().await {
        ...
        let resp = match cmd {
            ...
            // "map(|_| None)" transforms "Result<()>" to "Result<Option::None>"
...
impl BufferedClient {
    ...
    pub fn buffer(client: Client) -> BufferedClient {
        // Setting the message limit to a hardcoded value of 32. In a real app, the
    ...
    pub async fn get(&mut self, key: &str) -> Result<Option<Bytes>> {
        ...
        // initialize a new "oneshot" to receive the response back from the connection
